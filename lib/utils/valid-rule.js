...
/**
 * 校验规则
 * @version 2.4.0.210909    增加java类名相关校验
 * @changelog
 *      2.4.0.210909    增加java类名相关校验
 *      2.3.0.210909    增加cron相关校验
...
import { isNumber, isInteger, isDate, isEmpty, isDateTime, isCron, isJavaClass } from './sy-util/validate.js'
...
const Rule = {
    ...
    cronRequired(){
        return (rule, value, callback, source, options) => {
            var errors = []
            if (!isCron(value)){
                errors.push(new Error('需要为cron表达式'))
            }
            callback(errors)
        }
    },
    ...
    javaClassRequired(){
        return (rule, value, callback, source, options) => {
            ...
            if (!isJavaClass(value)){
                errors.push(new Error('需要为java类完全限定名'))
            }
            callback(errors)
        }
    },
     ...
     */
     javaClassOptional(){
        return (rule, value, callback, source, options) => {
            var errors = []
            if (!isEmpty(value) && !isJavaClass(value)){
                errors.push(new Error('需要为java类完全限定名'))
            }
            callback(errors)
        }
