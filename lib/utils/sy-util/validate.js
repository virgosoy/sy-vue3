/** input 字符串校验类 
    @version 1.5.1.210909   关闭某行eslint避免报错
    @changeLog  
        1.5.1.210909    关闭某行eslint避免报错
        1.5.0.210909    增加 isJavaClass
...
export function isCron (s) {
    ...
    // 下面的如果字符串都是空格或\t会返回true，故在上面判断拦截
 ...
 * 是否是java类的完全限定名
 * @param {string} s 
 * @version 1.5.0.210909
 * @since 1.5.0.210909
 */
export function isJavaClass (s) {
    if(isEmpty(s)){
        return false
    }
    // 下面代码如果传入 undefined 类型会被转成 'undefined' 字符串会校验通过，故在上面判断拦截
    // 包名与类名每个词均为 字母数字下划线组成，但开头不允许为数字。
